...
@Component({
  selector: 'app-disconnect-facebook',
  standalone: true,
  imports: [
    CommonModule,
    ContainerComponent,
  ...
  templateUrl: './disconnect-facebook.component.html',
  styleUrl: './disconnect-facebook.component.scss'
})
export class DisconnectFacebookComponent implements OnInit {
  constructor(
    ...
    response.subscribe({
      next: async (result: Result<Token>): Promise<void> => {
        if (result.success) {
          ...
          await this.snackService.showSnack(constants.MESSAGES.DISCONNECT_ACCOUNT.FACEBOOK.CHECK, 'OK');
        }
      },
      error: async () => {
